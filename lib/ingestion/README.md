...
2. Still 0 results after fix #1 with parameters matching the confirmed-working capture exactly. That second capture's Cookie header carries several cookies beyond `ci_session` — including one with an unusual, WAF/CDN-looking name (`UrtK5jLykjmM...`) — that `fetchDofSearchLive()` was silently dropping, forwarding only the isolated `ci_session` value. There was never actual proof `ci_session` alone is *sufficient*, only that it's *necessary* (the function throws if it's missing) — the "not a deliberate anti-bot challenge" framing from the original capture was true of the *headers* (no grc/igrc/xgrc), but never ruled out a cookie-based check. Now forwards the whole cookie jar from the initial GET's Set-Cookie, and both the GET and the POST send a real browser `User-Agent` (Node's fetch sends none by default) — same "make this indistinguishable from a real browser" reasoning, since the WAF/CDN-looking cookie makes a UA check plausible too.

Everything else in the captures (idOrg's `PE,PL,PJ,OA,EPEM,EF,OD,AV,CV,VG,TODOS`, the DataTables column/order boilerplate) already matched exactly. Not yet confirmed whether fix #2 actually resolves it — this is the next thing to watch on a live run.
