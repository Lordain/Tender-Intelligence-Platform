...
// Was "other-system" until 2026-09-20 on the grounds that it might well
// answer. It was asked, twice: the runner returned one challenge and two 502s,
// and the laptop returned three challenges — which reclassified those 502s as
// the same edge having a bad moment. Six attempts from two machines, nothing
// through. It is still another application with no parser, but that stopped
// being the binding constraint, so the verdict follows the measurement.
check("sisapinternet 实测拒绝我们", classifyAntaqHost("sisapinternet.antaq.gov.br").verdict, "refuses-us");
ok("理由里记着是哪天、几台机器测的", /2026-09-20/.test(classifyAntaqHost("sisapinternet.antaq.gov.br").why));
// The count is what a run reports; a verdict change must not quietly move it.
check("它仍然是 11 场，只是从「没解析器」变成「进不去」", triageHearingLinks(readFileSync(`${DIR}/01-em-andamento.html`, "utf8"), LIVE_INDEX).skipped.filter((s) => s.host === "sisapinternet.antaq.gov.br").length, 11);
