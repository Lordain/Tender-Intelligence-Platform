...
import { BILLING_INTERVAL_LABELS, TRIAL_DAYS, type BillingInterval } from "@/lib/access-control";
...
const PLANS: readonly Plan[] = [
  {
    id: "trial", eyebrow: "注册即享", name: `免费试用 ${TRIAL_DAYS} 天`,
  ...
  },
  {
    ...
    features: ["招标项目搜索与多行业组合筛选", "3 个账户登录", "查看全部招标项目", "完整标书详情与招投标时间", "不同账号可设置不同通知条件", "新标通知与邮件提醒（每日 2 个时段）"],
  },
] as const;
            ...
            <article key={plan.id} className={`flex flex-col overflow-hidden rounded-2xl border bg-[#fffdf9] shadow-[0_24px_60px_-48px_rgba(6,27,43,.5)] ${plan.id === "trial" ? "border-[#e7b84e]" : "border-[#d8e0e3]"}`}>
              <div className="border-b border-[#e2e7e9] p-6 sm:p-7">
                ...
                {plan.prices ? (
                  ...
                  <div className="mt-7 rounded-2xl bg-[#fff4d8] p-4 text-sm font-black text-[#8a5700]">{TRIAL_DAYS} 天内完整体验个人版全部功能</div>
                )}
              </div>
