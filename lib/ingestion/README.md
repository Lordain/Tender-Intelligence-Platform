...
Working hypothesis, not yet confirmed: `datos.gov.co`'s open-data mirror of SECOP II (`p6dx-8zbt`) is a SEPARATE system from the live `community.secop.gov.co` site the user reads directly, and may lag behind it by an unknown amount — meaning `estado_del_procedimiento`/`nombre_del_proveedor` for this specific process might genuinely still read as un-awarded in the open dataset even though the live site already shows it as decided, regardless of how many times our own refresh re-queries that same lagging source. Handed the user a direct diagnostic to settle this without going through our app at all: `https://www.datos.gov.co/resource/p6dx-8zbt.json?$where=referencia_del_proceso in ('101147') OR id_del_proceso in ('101147')` — if that raw response itself doesn't show the real award info, this is a genuine upstream data-lag limitation (same class as the deadline-sync gap already documented above), not a code bug. Not yet confirmed either way.
