...
const sections: PolicySection[] = [
  { id: "collection", title: "我们收集的信息", paragraphs: ["根据用户使用的功能，平台可能处理账户资料、收藏与筛选设置、访问日志、设备和浏览器信息，以及用户主动提交的支持请求。", "为了解产品使用情况，平台会生成随机匿名会话标识，并记录访问页面、所选筛选类别、项目查看以及收藏或取消收藏事件；运营统计不保存搜索框中输入的具体文字。"], items: ["账户信息，例如邮箱与身份验证状态。", "产品使用信息，例如收藏项目、筛选类别和通知偏好。", "安全与诊断信息，例如访问时间、错误日志和异常活动记录。"] },
  { id: "use", title: "信息的使用方式", paragraphs: ["相关信息用于提供账户功能、保存偏好、发送用户选择接收的提醒、形成汇总运营统计、保障系统安全、排查故障并改进服务。平台不会以与这些目的不相容的方式使用个人信息。"] },
...
export default function PrivacyPage() {
  return <PolicyPage eyebrow="Privacy" title="隐私政策" intro="本政策概述平台可能处理的信息、使用目的和用户可以作出的选择。" updated="2026年9月4日" sections={sections} />;
}
