...
**Fix**: a weekly Claude Code Routine (self-bound to this coding session, same pattern as the existing monthly FX-rate-refresh Routine) that re-runs `ingest:colombia-live` against the last 6 months' window, relies on the existing upsert-by-slug to refresh any row whose dynamic fields (submission deadline, status, awarded provider, award date/value) changed since it was first captured, and no-ops cleanly for anything unchanged — reviewed and pushed by the agent the same way the FX-rate Routine is, not given direct production write access.
