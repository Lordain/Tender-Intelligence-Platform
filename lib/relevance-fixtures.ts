...
export const RELEVANCE_FIXTURES: RelevanceFixture[] = [
  ...
  },
  {
    ...
    expectedTier: "standard",
    note: "The control for that pattern: a real description that merely ENDS in a company name must survive. Any procurement verb in the title means it says what is being bought. Tier moved significant -> standard on 2026-09-18 when the bands were rewritten: $3M is inside the new standard band ($1M-$5M). What this fixture is for — the row is not excluded as a bare company name — is untouched.",
    scopeType: "equipment",
    estimatedValue: 3_000_000,
    currency: "USD",
    country: "Colombia",
  ...
  },
  {
    ...
    expectedTier: "significant",
    note: "Guards the leading-fortalecimiento rule: the same word introduces real roadworks, which is why it carries a lookahead for the infrastructure nouns. flagship -> significant on 2026-09-18: COP 26bn is ~$8.28M, inside the new $5M-$10M band. The guard is that this is NOT excluded, which still holds.（2026-09-18 起：常规 $1M–$5M、中型 $5M–$10M、大型 $10M+；巴西下限 $2M）",
  ...
  },
  {
    ...
    expectedTier: "significant",
    note: "The row that produced the municipal-amenity value exception (user's call, 2026-09-11). Real value, COP 28,037,383,178 ≈ USD 8.9M — a structural works contract at a scale a foreign contractor bids on, which the flat amenity rule was excluding alongside a USD 2.5M skating rink. The three fixtures below are its guards: the exception must not widen the class. flagship -> significant on 2026-09-18 ($8.9M is inside the new $5M-$10M band). This one nearly broke silently: the exception read FLAGSHIP_VALUE_USD, so raising that band to $10M would have put this row back in the excluded pile and reversed the 2026-09-11 decision as a side effect. It now has its own constant, LARGE_WORKS_BUILD_USD, still $6,000,000.",
  ...
  },
  {
    ...
    expectedTier: "standard",
    note: "Peru PEC-NCPD-PROC-5-2026-MTC/21-3, real. S/ 18.1M of roadworks for PROVIAS Descentralizado, convocatoria 10/09 with registration open 11/09–21/09. Pinned as INCLUDED so a widening of the direct-award patterns cannot swallow it. significant -> standard on 2026-09-18: $4.835M is under the new $5M significant floor. Included is what this pins, and it still is.（2026-09-18 起：常规 $1M–$5M、中型 $5M–$10M、大型 $10M+；巴西下限 $2M）",
  ...
  },
  {
    ...
    expectedTier: "standard",
    note: "Real title (2026-09-18, per the user: 灌溉系统都加水工程的标签). The ASK was a tag, and the tag is pinned in test:industry-tags — this fixture exists because the tag changes the tier too: `water` is a target industry, so with a disclosed value the row is 中型 instead of falling through. With NO disclosed value it is still excluded by Peru's undisclosed-value rule, which is that rule working as specified, not this one failing. significant -> standard on 2026-09-18: $3.5M is inside the new standard band. The tag carrying the row past the industry gate is what this fixture is for, and that is unchanged.",
  ...
  },
  {
    ...
    expectedTier: "excluded",
    note: "Reversed 2026-09-25. This was written as the control that Santiago's Américo Vespucio Oriente motorway survives the drug rules, and it did — but the row is not the motorway. AIF is Asesoría a la Inspección Fiscal, and its own description says so: a contrato de asesoría to support the fiscal inspector of the concession. It is an inspection consultancy, and CHILE_NOT_A_TARGET_TITLE now excludes it by its opening word. The earlier note's point about carrying the summary in full still stands for every Chilean fixture.",
  ...
  },
  {
    ...
    expectedTier: "excluded",
    note: "Reversed 2026-09-25: a concession to COLLECT household waste is a municipal service, however large (19.2bn CLP), and 日常性服务 is out everywhere else on the platform. It was flagship only because Chilean rows carry scopeType 'unknown', so the scope-based service rules never saw it. Its other job — pinning the CLP rate with a priced flagship — passes to the Iquique CESFAM fixture below.",
  ...
  // CHILE_NOT_A_TARGET_TITLE (2026-09-25). Real open rows from the busca
  // door, summaries in full — see the AVO II note above for why that matters.
  {
    title: "CONSTRUCCIÓN CENTRO SALUD FAMILIAR COMUNA DE IQUIQUE",
    summary: "El objeto de la presente licitación es contratar la ejecución integral de la obra Construcción Centro de Salud Familiar Comuna de Iquique, Código BIP N.° 30108583-0, comprendiendo la construcción, provisión de materiales, equipamiento, mano de obra y todas las actividades necesarias para la correcta y oportuna entrega del establecimiento, conforme a la normativa vigente.",
    expectedTier: "flagship",
    note: "Real open row, 1057964-5-LR26: building a family health centre, 12.78bn CLP. Takes over the CLP-rate pin from the waste concession above — at 953 CLP/USD this is ~13.4M USD and flagship; a rate wrong by an order of magnitude moves it. Also the control that a real works contract is untouched by the Chilean title rules.",
    buyer: "SERVICIO DE SALUD IQUIQUE",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
    estimatedValue: 12782252140, currency: "CLP",
  },
  {
    title: "ADQUISICIÓN GRÚA HORQUILLA",
    summary: "Se requiere la adquisición de una grúa horquilla correspondiente al proyecto Adquisición Vehículos y Maquinarias Comuna de Chañaral, según especificaciones técnicas adjunta.",
    expectedTier: "standard",
    note: "Real open row, 2924-25-LE26: a forklift purchase. The control that an equipment purchase with no amount still stands after the Chilean rules — its keep signal lives in the SUMMARY, which is why these rules read the title and the classifier still reads both.",
    buyer: "I MUNICIPALIDAD DE CHANARAL",
    country: "Chile", scopeType: "unknown", governmentLevel: "municipal",
  },
  {
    title: "PPP, 34° LLAMADO, GRUPO 4, LP28 YUMBEL",
    summary: "Consiste en la construcción de las obras detalladas en las Especificaciones Técnicas y Planos correspondientes al proyecto “PPP 34° LLAMADO, GRUPO 4, LP28 YUMBEL”, debiendo entregarse totalmente terminadas y operativas, y ejecutarse de tal manera de minimizar los inconvenientes a los usuarios de las vías y habitantes del sector.",
    expectedTier: "excluded",
    note: "Real open row, 5407-38-O126. PPP here is SERVIU's Programa de Pavimentación Participativa — neighbourhood street paving — not a public-private partnership. An earlier comment in this file counted 'seven lots of the 34th PPP round' among the rows worth keeping; that was a misreading of the acronym.",
    buyer: "SERVICIO DE VIVIENDA Y URBANIZACION REGIÓN DEL BIO BIO",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
  },
  {
    title: "CONSERVACIÓN OBRA FISCAL EMBALSE LOS AROMOS 2026 REGIÓN DE VALPARAÍSO",
    summary: "OBRAS DE CONSERVACIÓN",
    expectedTier: "excluded",
    note: "Real open row, 1504-11-O126, stored flagship before this change on the word 'embalse'. MOP's CONSERVACIÓN is upkeep of an existing reservoir. 维护类都不要.",
    buyer: "MINISTERIO DE OBRAS PUBLICAS DIREC CION GRAL DE OO PP DCYF",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
  },
  {
    title: "CONSTRUCCIÓN SERVICIO SANITARIO RURAL PAILLAHUE, COMUNA DE HUALQUI",
    summary: "CONSTRUCCIÓN SERVICIO SANITARIO RURAL PAILLAHUE, COMUNA DE HUALQUI",
    expectedTier: "excluded",
    note: "Real open row, 1507-21-O126. A rural drinking-water system (SSR); the user deleted this class from the first live import.",
    buyer: "MINISTERIO DE OBRAS PUBLICAS DIREC CION GRAL DE OO PP DCYF",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
  },
  {
    title: "Adquisición de Albumina Humana por Convenio de Suministro para el Hospital Padre Alberto Hurtado",
    summary: "El objetivo de la presente licitación es proveer del suministro de Albumina Humana que cumpla con las exigencias precisadas en las Bases Administrativas y Técnicas de la Propuesta.",
    expectedTier: "excluded",
    note: "Real open row, 1058097-23-LR26, 2.29bn CLP. Human albumin — a blood product — under a convenio de suministro, which is how Chilean hospitals contract recurring consumables. Kept before only because its amount cleared the floor and nothing named it.",
    buyer: "HOSPITAL PADRE ALBERTO HURTADO",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
    estimatedValue: 2290000000, currency: "CLP",
  },
  {
    title: "SERV PROFESIONALES PARA ASESORÍA EN RIESGOS PSICOSOCIAL ACOSO Y VIOLENCIA EN EL TRABAJO PARA AFILIADOS EL ISL",
    summary: "El objetivo es contar con servicios profesionales de psicólogo/a para asesorar técnicamente a las entidades empleadoras adheridas al ISL en la prevención de riesgos psicosociales, acoso y violencia en el trabajo.",
    expectedTier: "excluded",
    note: "Real open row, 1415858-15-LR26. 'SERV' with no full stop — the first draft only knew 'SERV.' and let this professional-services contract through.",
    buyer: "INSTITUTO DE SEGURIDAD LABORAL",
    country: "Chile", scopeType: "unknown", governmentLevel: "federal",
    estimatedValue: 980000000, currency: "CLP",
  },
  {
    title: "SERVICIO INTEGRAL DE TELEVIGILANCIA Y GUARDIAS",
    summary: "LA I. MUNICIPALIDAD DE CONCON REQUIERES LA CONTRATACION Y EJECUCION DEL SERVICIO INTEGRAL DE TELEVIGILANCIA Y GUARDIAS PRIVADOS PARA DEPENDENCIAS DE LA I. MUNICIPALIDAD DE CONCON, SEGUN BASES TECNICAS Y DEMAS ANTECEDENTES QUE COMPONEN LA PROPUESTA.",
    expectedTier: "excluded",
    note: "Real open row, 2597-27-LR26, 3.66bn CLP. Guards and remote monitoring for a municipality: a service however large.",
    buyer: "I MUNICIPALIDAD DE CONCON",
    country: "Chile", scopeType: "unknown", governmentLevel: "municipal",
  ...
  },
  {
    ...
    expectedTier: "excluded",
    note: "Real title (2026-09-18, user: 这项目金额 US$973,907 但被标为中型项目？). It was 中型, and not because of the sports words — ANY works contract above the $800,000 floor was, because `construcción` matched FLAGSHIP_INDUSTRY_KEYWORDS and that promoted on a disclosed value of any size. SIGNIFICANT_VALUE_USD was unreachable as a band. Now the bands decide whenever an amount is disclosed. 2026-09-18: the floor moved to $1,000,000 and $973,907 is under it, so the row the user queried as wrongly-中型 is now out of the feed entirely.",
