...
**Not yet exercised against the real endpoint from this session** — this sandbox has no network egress to *.gob.mx, so the very first live run needs to happen from the user's own machine and be watched once before trusting it at scale, same caveat every other "not yet live-tested" connector in this file carries. The request shape itself isn't a guess — it's copied field-for-field from the user's own real "Copy as cURL" capture. One open question the capture didn't pin down: the exact `fechaIni`/`fechaFin` date-string format DOF's search form itself sends (the form field says to type it exactly as shown on the DOF search page, rather than guessing a separator/order and silently getting it wrong) — worth confirming against a real capture's body if the first live run returns zero results unexpectedly.
...
Deliberately did NOT restore "ADQUISICIÓN DE MAQUINARIA PESADA" (heavy machinery) to the whitelist even though it shares the same `vehicles` industry tag and was reversed to "excluded" alongside the vehicle example in the Sixth pass — the user's ask was specifically vehicles (cars/buses/trucks/SUVs), and "maquinaria pesada" isn't a noun the new pattern matches, so it stays excluded, kept as-is in the fixture file.
