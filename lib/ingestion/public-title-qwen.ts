...
- 但括号里如果装的是中文、而且和括号前面的字一样（例如「华胡阿潘-瓦哈卡（华胡阿潘-瓦哈卡）」），整个括号删掉。括号的作用是给出原文拼写，重复一遍中文没有任何意义。
...
写什么：只写 titleZh 和 summaryZh 里真的说了的内容——是一类什么采购、包含哪些工作内容、大致规模。

【最重要的一条】原文没说的，一个字都不许写。
读者会据此判断要不要投标，编出来的工作内容比写得短要糟得多。具体地说：
- 原文只写「高中建设工程」，就不许写成「包含教学楼、实验楼、行政办公楼及操场看台」——我们并不知道有没有实验楼和看台。
- 不许补「需满足国家某某技术标准」「需具备某某资质」「按总价合同实施」「分阶段实施」，除非原文写了。
...
输出：
  ...
  summaryZhPublic：城市环路变电站的扩建工程，包含34.5kV开关柜安装、控制保护系统改造以及配套土建施工。
...
输出：
  ...
  summaryZhPublic：州级公路约七公里路段的路面修复与新建工程，由承包方包工包料实施。
