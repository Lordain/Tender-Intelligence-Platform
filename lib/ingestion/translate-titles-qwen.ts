...
- Put the Spanish in full-width parentheses after a transliterated place, facility or project name: 马塔德罗（Matadero）泵站, 阿瓜弗里亚（Agua Fría）泉, 劳雷莱斯二期（Laureles II）. This name is the one thing in the title a bidder must match against a map and against the bid documents, and a transliteration on its own appears in neither — 阿瓜弗里亚 is unsearchable, Agua Fría is not.
  ...
  Copy the name inside the parentheses exactly as the Spanish writes it, letter for letter. It is there to be searched for, so a re-spelling is worse than omitting it: CATACAOS rendered 卡塔考斯（Catacos）matches no map and no document while reading as authoritative. Normalising case and accents is fine — RIO MEZCALAPA earning （Río Mezcalapa）is correct — but never add, drop or change a letter. If you are unsure of the spelling, leave the parenthesis out.
