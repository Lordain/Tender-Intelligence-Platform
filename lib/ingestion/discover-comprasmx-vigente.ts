...
export type DiscoverComprasMxVigenteResult = {
   ...
   * read-only deploy) — or when this was a preview, since the export now
   * happens inside upsertTendersBatched, which a preview never reaches.
   */
...
export async function discoverComprasMxVigente(
  ...
  const { upsertedCount, skippedExcludedCount, protectedCount, skippedManuallyDeletedCount, failed, excludedCsvPath } = await upsertTendersBatched(
    supabase,
    recent,
    (done, total) => console.log(`[discover-comprasmx-vigente] Upserted ${done}/${total}...`),
  );
  return { ...result, upsertedCount, skippedExcludedCount, protectedCount, skippedManuallyDeletedCount, failed, excludedCsvPath };
}
