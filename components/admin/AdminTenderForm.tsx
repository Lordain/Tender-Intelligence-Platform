    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        {isEdit && (
          <div className="border-t border-[#e5e9eb] pt-5">
            ...
            <div className="mt-3">
              ...
              {/* Only where a schedule page exists to copy from, and the form
                  names the right one: Peru's SEACE ficha de selección, and
                  Mexico's Proyectos Estratégicos procedure page. Offering it
                  elsewhere tells an admin to go copy a page that does not
      ...
      */}
      <FormSection title="标书分析结果" description="资质要求、经验要求、所需文件与风险提示——通常由文件分析流程生成，也可以在这里手动补充或修正；每项立即保存。">
        {/*
          Explicit user request, 2026-09-06: a one-line "what is this
          tender" summary shown on the public page directly above 资质
          要求 — normally generated by document analysis (see extract-
          requirements.ts's oneLineSummary field) alongside qualifications/
          risks, but a plain tenders column, so also directly editable by
          hand here, same placement (right above 资质要求) as the public
          page. Unlike the RequirementsEditor/RisksEditor blocks below,
          this is a plain form field saved with the rest of the form (not
