...
**Live-tested and fixed (2026-09-04)** — the first real run (from the user's own machine, this sandbox still has no egress to *.gob.mx) returned 0 results for a search the user then confirmed by hand DOES have real hits (CFE, the same window). Root cause: `fechaIni`/`fechaFin` were being sent as `DD/MM/YYYY` (slashes) — a guess based on this project's OTHER DOF date convention (`dof-notice-detail.ts`'s URL param). The real search form uses **`DD-MM-YYYY` with hyphens** — confirmed directly from the user's own manual search on the live page (the "Desde"/"Hasta" fields and the resulting "Búsqueda realizada... desde 03-08-2026 hasta 04-09-2026" confirmation text both showed it). `ImportDofSearchForm.tsx`'s `isoToDofDate()` now emits hyphens. Everything else in the captured request (idOrg's `PE,PL,PJ,OA,EPEM,EF,OD,AV,CV,VG,TODOS`, the DataTables column/order boilerplate) matched the user's real page exactly, so this one separator was the whole bug.
