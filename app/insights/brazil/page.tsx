      ...
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[15rem_minmax(0,1fr)]">
          ...
          <div className="min-w-0 space-y-8">
            ...
            <section id="sources" className="scroll-mt-8 rounded-3xl border border-[#dbe2e5] bg-[#fffdf9] p-6 sm:p-8"><p className="text-xs font-black uppercase tracking-[0.18em] text-[#2d7450]">Official sources</p><h2 className="mt-3 text-2xl font-black">资料来源与汇率说明</h2><div className="mt-6 grid gap-3">{sources.map((source)=><a key={source.href} href={source.href} target="_blank" rel="noreferrer" className="group rounded-xl border border-[#dbe2e5] px-4 py-4 transition hover:border-[#2d7450] hover:bg-[#eef6f1]"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-black">{source.label}</p><p className="mt-1 text-xs leading-6 text-[#71808a]">{source.note}</p></div><span className="shrink-0 font-black text-[#2d7450]">↗</span></div></a>)}</div><div className="mt-6 rounded-xl border border-[#dbe2e5] bg-[#f4f6f6] px-4 py-4 text-xs leading-6 text-[#71808a]">本文人民币以外的美元换算统一采用巴西中央银行2026年9月18日PTAX收盘卖出价：1美元＝5.1575巴西雷亚尔。换算值四舍五入，仅用于理解规模，不代表项目结算汇率。规划、项目数量和实施进度会调整，具体参与条件以主管机构和项目最新文件为准。</div></section>
          </div>
        </div>
      </main>
    </article>
