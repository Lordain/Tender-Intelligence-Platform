...
**Explicitly NOT acted on this pass** — a real, broader-blast-radius finding flagged for the user rather than fixed unilaterally: several PEMEX-buyer tenders (e.g. "ADQUISICIÓN DE EXCAVADORA HIDRÁULICA PARA USARSE EN LA REFINERÍA MADERO...", "ADQUISICIÓN DE CAMIÓN TIPO CISTERNA...PARA LA REFINERÍA MINATITLÁN") show an "能源" (energy) tag that comes from the BUYER NAME containing "pemex" (`\bpemex\b` in the energy pattern), not from the procured item itself (an excavator, a tanker truck) having anything to do with energy/oil&gas. This is the SAME buyer-name-inflation problem already fixed once this session — but that fix only touched the internal `contentIndustries` recompute inside `classifyRelevance()`'s allowlist gate (title/summary only, no buyer), not the STORED `industries` array every mapper still computes WITH the buyer name included, which is what the tender-card badges actually render. Fixing this properly means deciding whether buyer name should stay in the stored/displayed tags at all — a bigger, more consequential change (affects every already-tagged PEMEX-buyer row, not just these two) than a scoped keyword tweak, so it's flagged here rather than changed without asking.
